...
def get_font_size(font: ImageFont.FreeTypeFont, text: str) -> Tuple[int, int]:
    """Get font width and height for a given text (Pillow 10+ compatible)."""
...
def gerar_data(img: Image.Image, cor_texto2: Tuple[int, ...]) -> Image.Image:
    ...
    altura_i = 972
    w, h = get_font_size(fonte_titulo, data)
...
def pegar_identidade_visual(titulo_cardapio: str) -> Tuple[str, str, Tuple[int, ...], Tuple[int, ...], Tuple[int, ...]]:
    ...
    if "tradicional" in titulo_cardapio.lower():
        pasta = "vermelho"
        return (
            str(ASSETS_DIR / pasta / "img_cardapio"),
            str(ASSETS_DIR / pasta / "img_obs"),
            (0, 0, 0),
            (255, 255, 255),
            (174, 34, 37),
        )
...
def gerar_imagem_cardapio(
    ...
    for i, texto in enumerate(textos):
        ...
        if len(wrap_textos) == 0:
            continue
        for wrap_texto in wrap_textos:
            w, h = get_font_size(fonte_texto, wrap_texto)
            img_draw.text(
                ((IMG_SIZE / 2) - comprimento_fonte_i, altura_f), wrap_texto, font=fonte_texto, fill=cor_texto1
            )
            altura_f += h + espacamento_vertical_fonte
        ...
        img_draw.bitmap(
            (
                (IMG_SIZE / 2) - comprimento_bolinha_i,
                ((altura_f - ultima_altura - bolinha.size[0]) / 2) + ultima_altura,
            ),
            bolinha,
            fill=cor_auxiliar,
        )
        if i < len(textos) - 3:
            img_draw.line(
                [
                    ((IMG_SIZE / 2) - posicao_linha_horizontal, altura_f),
                    ((IMG_SIZE / 2) + posicao_linha_horizontal, altura_f),
                ],
                fill=cor_auxiliar,
                width=3,
    ...
    img_draw.line(
        [
            ((IMG_SIZE / 2) - comprimento_linha_vertical_i, altura_i),
...
def gerar_imagem_obs(
    ...
    for j, texto in enumerate(textos):
        ...
        for linha in wrap_texto:
            w, h = get_font_size(fonte_texto, linha)
